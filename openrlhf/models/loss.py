...
class QwenPRMLoss(nn.Module):
    """
    Process Reward Model Loss
    """

    def __init__(self, prm_sep_token_id: int, reward_tokens: Optional[list[int]] = None):
        super().__init__()
        self.IGNORE_INDEX = -100
        self.loss = nn.CrossEntropyLoss(ignore_index=self.IGNORE_INDEX)
        self.reward_tokens = reward_tokens
        self.prm_sep_token_id = prm_sep_token_id

    def forward(self, inputs: torch.Tensor, logits: torch.Tensor, labels: torch.Tensor, *, return_acc: bool = False):
        step_sep_mask = inputs == self.prm_sep_token_id
        logits = logits[step_sep_mask]  #for qwen-prm, the dim 1 present positive
        # labels = labels[placeholder_mask]
        loss = self.loss(logits, labels)
        if not return_acc:
            return loss
        ...
        if labels.dtype == logits.dtype:
            labels = labels.argmax(dim=-1)
        logit_label = logits.argmax(dim=-1)
        acc = (logits.argmax(dim=-1) == labels).float().mean()
        return loss, acc
    

class PUREPRMLoss(nn.Module):
    """
    Process Reward Model Loss
    """
    ...
    def forward(self, prm_token_masks: torch.Tensor, logits: torch.Tensor, labels: torch.Tensor, *, return_acc: bool = False):
        logits = logits[prm_token_masks != 0]  #for qwen-prm, the dim 1 present positive
        # labels = labels[placeholder_mask]
