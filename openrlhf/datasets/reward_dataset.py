    ...
    if apply_chat_template:
        if prompt_key:
            ...
            chosen_prompt = [
                {"role": "system", "content": SYSTEM},
                {"role": "user", "content": data[prompt_key]},
                {"role": "assistant", "content": data[chosen_key]},
            ]
            rejected_prompt = [
                {"role": "system", "content": SYSTEM},
                {"role": "user", "content": data[prompt_key]},
                {"role": "assistant", "content": data[rejected_key]},
            ]

            prompt = apply_chat_template(messages_prompt, tokenize=False, add_generation_prompt=True)
            chosen = apply_chat_template(chosen_prompt, tokenize=False)[len(prompt) :]
            rejected = apply_chat_template(rejected_prompt, tokenize=False)[len(prompt) :]
